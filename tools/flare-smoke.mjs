...
await new Promise(resolve=>server.listen(4319,'127.0.0.1',resolve));let browser,page;
...
try{
 ...
 page=await context.newPage();page.on('pageerror',error=>errors.push(error.message));page.on('request',r=>{if(!r.url().startsWith('http://127.0.0.1:4319/'))external.push(r.url());});
 ...
 console.log('BROWSER_STEP ready');await page.screenshot({path:path.join(evidence,'overview.png')});
 await page.click('#start');await page.waitForFunction(()=>window.__flare.snapshot().ticks>=110,null,{timeout:7000});
 const active=await page.evaluate(()=>window.__flare.snapshot());assert.ok(active.camera.s>.25);assert.equal(active.metrics.blockedMoves,0);checks.push('closer camera and legal movement');console.log('BROWSER_STEP moving',active.ticks);
 ...
 await page.setViewportSize({width:844,height:390});await page.waitForTimeout(80);await page.setViewportSize({width:390,height:844});checks.push('rotate/resize during run without reset');console.log('BROWSER_STEP rotated',await page.evaluate(()=>({tick:window.__flare.snapshot().ticks,phase:document.querySelector('#phase').textContent,visibility:document.visibilityState,pause:document.querySelector('#pause').textContent})));
...
}catch(error){console.error('BROWSER_FAILURE',error.message,JSON.stringify({errors}));if(page)console.error('BROWSER_STATE',JSON.stringify(await page.evaluate(()=>({state:window.__flare?.snapshot(),phase:document.querySelector('#phase')?.textContent,visibility:document.visibilityState,pause:document.querySelector('#pause')?.textContent})).catch(()=>null)));throw error;}finally{await browser?.close();server.closeAllConnections();await new Promise(resolve=>server.close(resolve));}
