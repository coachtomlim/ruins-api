...
export function localPath(requestUrl){
  ...
  if(file!==root&&!file.startsWith(root+path.sep))throw Error('outside public root');
  return file;
}
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url))server.listen(port,'127.0.0.1',()=>console.log(`S8A preview http://127.0.0.1:${port}/quick-dungeon/flare-s8a/`));
