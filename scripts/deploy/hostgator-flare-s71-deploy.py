#!/usr/bin/env python3
"""Bounded WEB-FLARE S7.1 HostGator deployer. Writes only flare-s71 and /k."""
...
REPO_ROOT=Path(__file__).resolve().parents[2];SOURCE_SHA='c4834520570adc72b02e50aa6a0f2a764030989c'
S7_SHA='4ca60cab1d14b8f7cc98087c49a72ad132310942';S6_SHA='490e21b8cfe358fb96b933705d3b4206840a9305';S5_SHA='ecbbb4f10c2fbfab992c0bf77cdb9ac996333c3e';S4_SHA='a038c163530ae55ab8d6a158591443c84ebe8dde';S3_SHA='3c1e62c82ab4277956c451ad8a396cea11b1f4a9';S2_SHA='8cc57ddacfc2e12ac65b120496f1ee8915e4525b'
S71_ROOT='public_html/quick-dungeon/flare-s71';SHORT_ROOT='public_html/k';ALLOWED=(S71_ROOT,SHORT_ROOT)
FILES=['public/flare-s71/index.html','public/flare-s71/builder.mjs','public/flare-s71/flow.mjs','public/flare-s71/challenge.html','public/flare-s71/challenge.mjs','public/flare-s71/hero-preview.mjs','public/flare-s71/game.mjs','public/flare-s71/calibration.mjs','public/flare-s71/rooms.mjs','public/flare-s71/actors.mjs','public/flare-s71/simulation.mjs','public/flare-s71/renderer.mjs','public/flare-s71/style.css']
HTACCESS='''DirectoryIndex index.html\nAddDefaultCharset UTF-8\nAddType text/html .html\nAddType text/css .css\nAddType text/javascript .js .mjs\n<IfModule mod_headers.c>\nHeader set X-Content-Type-Options "nosniff"\n</IfModule>\n'''
SHORT_HTACCESS='''RewriteEngine On\nRewriteRule ^([A-Za-z0-9_-]{4})/?$ /quick-dungeon/flare-s71/challenge.html [L]\nOptions -Indexes\n'''
FROZEN=[(S2_SHA,'public/flare-s2/index.html','/quick-dungeon/flare-s2/'),(S2_SHA,'public/flare-s2/stock.mjs','/quick-dungeon/flare-s2/stock.mjs'),(S3_SHA,'public/flare-s3/index.html','/quick-dungeon/flare-s3/'),(S3_SHA,'public/flare-s3/actors.mjs','/quick-dungeon/flare-s3/actors.mjs'),(S4_SHA,'public/flare-s4/index.html','/quick-dungeon/flare-s4/'),(S4_SHA,'public/flare-s4/game.mjs','/quick-dungeon/flare-s4/game.mjs'),(S5_SHA,'public/flare-s5/index.html','/quick-dungeon/flare-s5/'),(S5_SHA,'public/flare-s5/challenge.mjs','/quick-dungeon/flare-s5/challenge.mjs'),(S6_SHA,'public/flare-s6/index.html','/quick-dungeon/flare-s6/'),(S6_SHA,'public/flare-s6/challenge.mjs','/quick-dungeon/flare-s6/challenge.mjs'),(S7_SHA,'public/flare-s7/index.html','/quick-dungeon/flare-s7/'),(S7_SHA,'public/flare-s7/challenge.mjs','/quick-dungeon/flare-s7/challenge.mjs')]
...
def guard(path):
 ...
 if not any(norm==root or norm.startswith(root+'/') for root in ALLOWED):fail(f'refusing path outside S7.1 roots: {path}')
def headers():
 if not TOKEN:fail('CPANEL_API_TOKEN is not set')
 return {'Authorization':f'cpanel {CPANEL_USER}:{TOKEN}','User-Agent':'QuickDungeonS71Deploy/1.0','Accept':'application/json'}
...
def verify_source():
 ...
 if changed!=0:fail(f'S7.1 deploy files differ from authorised source {SOURCE_SHA}')
def public_get(url):
 req=Request(url,headers={'User-Agent':'Mozilla/5.0 QuickDungeonS71Probe/1.0','Cache-Control':'no-cache'})
 try:
...
def frozen_gate():
 ...
 for url,marker in [('/q/hiS4',b'RUN THE GAUNTLET'),('/q/Rind',b'RUN THE GAUNTLET'),('/g/MsJ9',b'HOW TO PLAY'),('/h/UvVY',b'HOW TO PLAY'),('/j/UvVY',b'HOW TO PLAY')]:expect(PUBLIC_BASE+url,{'text/html'},marker)
def deploy():
 ensure_dir(S71_ROOT);ensure_dir(SHORT_ROOT);save(f'{S71_ROOT}/.htaccess',HTACCESS);save(f'{SHORT_ROOT}/.htaccess',SHORT_HTACCESS)
 for rel in FILES:
  local=REPO_ROOT/rel
  if not local.is_file():fail(f'missing source file: {rel}')
  save(f'{S71_ROOT}/{rel.removeprefix("public/flare-s71/")}',local.read_text(encoding='utf-8'))
def s71_gate():
 base=PUBLIC_BASE+'/quick-dungeon/flare-s71/';expect(base,{'text/html'},b'S7.1');expect(base+'builder.mjs',{'text/javascript','application/javascript'},b'makeInviteUrl');expect(base+'challenge.html?demo=1',{'text/html'},b'DUNGEON RUNNER');expect(base+'challenge.mjs',{'text/javascript','application/javascript'},b'startComposedHeroStance');expect(base+'hero-preview.mjs',{'text/javascript','application/javascript'},b'animations?.stance');expect(base+'flow.mjs',{'text/javascript','application/javascript'},b'/k/');expect(base+'style.css',{'text/css'},b'invite-hero-card');expect(PUBLIC_BASE+'/k/UvVY',{'text/html'},b'DUNGEON RUNNER')
def main():
 mode=sys.argv[1] if len(sys.argv)==2 else ''
 if mode not in {'auth','probe','deploy'}:fail('usage: hostgator-flare-s71-deploy.py auth|probe|deploy')
 verify_source();uapi('Fileman','list_files',{'dir':'public_html'});print('AUTH PASS')
 if mode=='auth':return
 frozen_gate();print('S2-S7 AND /q /g /h /j FINGERPRINTS PASS')
 if mode=='probe':print('PROBE PASS');return
 deploy();s71_gate();frozen_gate();print('TEST PASS');print(f'S7.1 SOURCE: {SOURCE_SHA}');print(f'BUILDER URL: {PUBLIC_BASE}/quick-dungeon/flare-s71/');print(f'S7.1 INVITE: {PUBLIC_BASE}/k/UvVY');print('S2-S7 AND /q /g /h /j: PRESERVED')
