#!/usr/bin/env python3
"""Deploy WEB-FLARE S5 without modifying accepted S2/S3/S4 or /q routes.
...
REPO_ROOT=Path(__file__).resolve().parents[2];SOURCE_SHA='ecbbb4f10c2fbfab992c0bf77cdb9ac996333c3e';S4_SOURCE_SHA='a038c163530ae55ab8d6a158591443c84ebe8dde';S3_SOURCE_SHA='3c1e62c82ab4277956c451ad8a396cea11b1f4a9';S2_SOURCE_SHA='8cc57ddacfc2e12ac65b120496f1ee8915e4525b'
S5_ROOT='public_html/quick-dungeon/flare-s5';SHORT_ROOT='public_html/g';ALLOWED=(S5_ROOT,SHORT_ROOT)
FILES=['public/flare-s5/index.html','public/flare-s5/builder.mjs','public/flare-s5/flow.mjs','public/flare-s5/game.mjs','public/flare-s5/challenge.html','public/flare-s5/challenge.mjs','public/flare-s5/style.css','public/flare-s5/data/game.json']
HTACCESS='''DirectoryIndex index.html\nAddDefaultCharset UTF-8\nAddType text/html .html\nAddType text/css .css\nAddType text/javascript .js .mjs\nAddType application/json .json\n<IfModule mod_headers.c>\nHeader set X-Content-Type-Options "nosniff"\n</IfModule>\n'''
SHORT_HTACCESS='''RewriteEngine On\nRewriteRule ^([A-Za-z0-9_-]{4})/?$ /quick-dungeon/flare-s5/challenge.html [L]\nOptions -Indexes\n'''
...
def guard(path):
 ...
 if not any(norm==root or norm.startswith(root+'/') for root in ALLOWED):fail(f'refusing path outside S5 roots: {path}')
def headers():
 if not TOKEN:fail('CPANEL_API_TOKEN is not set')
 return {'Authorization':f'cpanel {CPANEL_USER}:{TOKEN}','User-Agent':'QuickDungeonS5Deploy/1.0','Accept':'application/json'}
...
def verify_source():
 ...
 if changed!=0:fail(f'S5 deploy files differ from authorised source {SOURCE_SHA}')
def public_get(url):
 req=Request(url,headers={'User-Agent':'Mozilla/5.0 QuickDungeonS5Probe/1.0','Cache-Control':'no-cache'})
 try:
...
def main():
 verify_source();uapi('Fileman','list_files',{'dir':'public_html'});ensure_dir(S5_ROOT);ensure_dir(SHORT_ROOT);save(f'{S5_ROOT}/.htaccess',HTACCESS);save(f'{SHORT_ROOT}/.htaccess',SHORT_HTACCESS)
 for rel in FILES:
  local=REPO_ROOT/rel
  if not local.is_file():fail(f'missing source file: {rel}')
  save(f'{S5_ROOT}/{rel.removeprefix("public/flare-s5/")}',local.read_text(encoding='utf-8'))
 base=PUBLIC_BASE+'/quick-dungeon/flare-s5/'
 expect(base,{'text/html'},b'Challenge a friend');expect(base+'builder.mjs',{'text/javascript','application/javascript'});expect(base+'flow.mjs',{'text/javascript','application/javascript'});expect(base+'game.mjs',{'text/javascript','application/javascript'});expect(base+'challenge.html?demo=1',{'text/html'},b'HOW TO PLAY');expect(base+'challenge.mjs',{'text/javascript','application/javascript'});expect(base+'style.css',{'text/css'});expect(base+'data/game.json',{'application/json'},b'warrior-l3');expect(PUBLIC_BASE+'/g/Ilyj',{'text/html'},b'HOW TO PLAY')
 # Existing accepted lines and /q remain dependencies/regression checks only, never writes.
 expect(PUBLIC_BASE+'/q/hiS4',{'text/html'},b'RUN THE GAUNTLET');expect(PUBLIC_BASE+'/q/Rind',{'text/html'},b'RUN THE GAUNTLET');expect(PUBLIC_BASE+'/quick-dungeon/flare-s4/game.mjs',{'text/javascript','application/javascript'});expect(PUBLIC_BASE+'/quick-dungeon/flare-s3/actors.mjs',{'text/javascript','application/javascript'});expect(PUBLIC_BASE+'/quick-dungeon/flare-s2/stock.mjs',{'text/javascript','application/javascript'})
 print('TEST PASS');print(f'S5 SOURCE: {SOURCE_SHA}');print(f'S4 PRESERVED: {S4_SOURCE_SHA}');print(f'S3 PRESERVED: {S3_SOURCE_SHA}');print(f'S2 PRESERVED: {S2_SOURCE_SHA}');print(f'BUILDER URL: {base}');print(f'DEFAULT INVITE: {PUBLIC_BASE}/g/Ilyj');print('LEGACY /q ROUTES: PRESERVED')
