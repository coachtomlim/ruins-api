#!/usr/bin/env python3
"""Bounded WEB-FLARE S8B HostGator updater. Updates only the verified existing flare-s8b root."""
...
SOURCE_SHA = '73c1ceed8ffbcec024d8e17c9c27b1e2ae9fc898'
PREVIOUS_SHA = 'e320917a49f35a71cf00033f7c31842da1e9b93a'
...
PREVIOUS_GIT_FILES = list(GIT_FILES)

EXPECTED_ROOT_NAMES = {
    '.htaccess','account-adapter.mjs','account-app.mjs','account-ready-view.mjs','account.css',
    'config.js','index.html','practice-app.mjs','practice-runner-catalog.mjs',
    'practice-runner-snapshot.mjs','practice.css','practice.html','stat-purchase-flow.mjs',
    'supabase-browser.mjs','vendor'
}
...
def headers() -> dict[str, str]:
    ...
    return {
        ...
        'User-Agent': 'QuickDungeonS8BUpdate/3.0',
...
def s8b_gate() -> None:
    ...
    for js_file in ('config.js', 'vendor/supabase.js', 'account-app.mjs', 'account-ready-view.mjs',
                    ...
                    'practice-runner-snapshot.mjs', 'practice-runner-catalog.mjs', 'practice-app.mjs'):
...
def main() -> None:
    ...
    if mode not in {'auth', 'probe', 'deploy'}:
        fail('usage: hostgator-flare-s8b-update-003.py auth|probe|deploy')
    ...
    target_baseline_gate()
    print('S8B CURRENT LIVE BASELINE: VERIFIED')
    ...
    print(f'S8B PREVIOUS SOURCE: {PREVIOUS_SHA}')
