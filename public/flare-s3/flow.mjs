...
export const S3_VERSION='web-flare-s3-0.2.0';
...
export function safeSender(value){
  const cleaned=String(value||'Buddy').replace(/[<>]/g,'').trim().slice(0,32);
  return cleaned||'Buddy';
}

export function makePlayerUrl(baseUrl,challenge,{sender='Buddy'}={}){
  const url=new URL('play.html',baseUrl);
  url.searchParams.set('from',safeSender(sender));
  url.hash='c='+encodeChallenge(challenge);
  return url;
}

export function inviteSender(search){
  return safeSender(new URLSearchParams(search||'').get('from')||'A friend');
}
