...
export function runnerSummary(profile,catalog){
  ...
  return Object.freeze({id:profile.id,name:profile.name,className:profile.className,level:profile.level,weapon:profile.weapon,hero:profile.hero,hp:hero.maxHp,attack:hero.damage,defense:hero.armor||0});
}
...
export function inviteSender(search){return safeSender(new URLSearchParams(search||'').get('from')||'A friend')}
...
export function encodeGauntletCode({roomId,enemyTypes=DEFAULTS.enemyTypes,potion=DEFAULTS.potion,targetHp=DEFAULTS.targetHp}){
  ...
  const payload=((1&3)<<14)|(room<<12)|(enemies[0]<<10)|(enemies[1]<<8)|(enemies[2]<<6)|((potion?1:0)<<5)|targetIndex;
  return encode24((payload<<8)|checksum16(payload));
}

export function decodeGauntletCode(code){
  const packed=decode24(code),payload=packed>>>8,check=packed&255;if(check!==checksum16(payload))throw Error('Gauntlet code checksum failed');
  const version=(payload>>>14)&3;if(version!==1)throw Error('Unsupported gauntlet code version');
  const room=(payload>>>12)&3;if(room>=ROOM_IDS.length)throw Error('Unknown room in gauntlet code');
  const enemyTypes=[(payload>>>10)&3,(payload>>>8)&3,(payload>>>6)&3].map(v=>{if(v>=TYPES.length)throw Error('Unknown enemy in gauntlet code');return TYPES[v]});
  const potion=Boolean((payload>>>5)&1),targetIndex=payload&31;if(targetIndex>18)throw Error('Invalid target in gauntlet code');
  return Object.freeze({roomId:ROOM_IDS[room],enemyTypes,potion,targetHp:(targetIndex+1)*5});
}
...
export function makePlayerUrl(baseUrl,options,{sender='Buddy'}={}){
  ...
  if(base.hostname==='think-2-thrive.com'||base.hostname.endsWith('.think-2-thrive.com')){
    const url=new URL(`/q/${code}`,base.origin);if(name!=='Buddy')url.searchParams.set('from',name);return url;
  }
  const url=new URL('play.html',base);url.searchParams.set('g',code);if(name!=='Buddy')url.searchParams.set('from',name);return url;
}

export function gauntletCodeFromLocation(locationLike){
  const pathname=String(locationLike?.pathname||''),m=pathname.match(/\/q\/([A-Za-z0-9_-]{4})\/?$/);if(m)return m[1];
  return new URLSearchParams(String(locationLike?.search||'')).get('g')||'';
}
