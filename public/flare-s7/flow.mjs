import {runnerIds} from '../flare-s5/game.mjs';
...
export function makeInviteUrl(baseUrl,invite,model,{sender='Buddy'}={}){
  ...
  if(base.hostname==='think-2-thrive.com'||base.hostname.endsWith('.think-2-thrive.com')){const url=new URL(`/j/${code}`,base.origin);if(name!=='Buddy')url.searchParams.set('from',name);return url}
  const url=new URL('challenge.html',base);url.searchParams.set('j',code);if(name!=='Buddy')url.searchParams.set('from',name);return url;
}
export function inviteCodeFromLocation(locationLike){const pathname=String(locationLike?.pathname||''),m=pathname.match(/\/j\/([A-Za-z0-9_-]{4})\/?$/);if(m)return m[1];return new URLSearchParams(String(locationLike?.search||'')).get('j')||''}
