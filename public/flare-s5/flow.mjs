import {runnerIds} from './game.mjs';
...
function checksum14(payload){let x=(payload^0x2A5)>>>0;x=((x*109)+37)>>>0;x^=x>>>7;x=((x*53)+11)>>>0;return x&0x3fff}
...
export function encodeInviteCode({runnerId,targetHp},model){
  ...
  const payload=(1<<7)|(runner<<5)|targetIndex;
...
export function decodeInviteCode(code,model){
  ...
  const version=(payload>>>7)&7;if(version!==1)throw Error('Unsupported invitation code version');
  ...
  return Object.freeze({runnerId:ids[index],targetHp:(targetIndex+1)*5,codeVersion:1});
}
export function makeInviteUrl(baseUrl,invite,model,{sender='Buddy'}={}){
  ...
  if(base.hostname==='think-2-thrive.com'||base.hostname.endsWith('.think-2-thrive.com')){const url=new URL(`/g/${code}`,base.origin);if(name!=='Buddy')url.searchParams.set('from',name);return url}
  const url=new URL('challenge.html',base);url.searchParams.set('g',code);if(name!=='Buddy')url.searchParams.set('from',name);return url;
}
export function inviteCodeFromLocation(locationLike){const pathname=String(locationLike?.pathname||''),m=pathname.match(/\/g\/([A-Za-z0-9_-]{4})\/?$/);if(m)return m[1];return new URLSearchParams(String(locationLike?.search||'')).get('g')||''}
