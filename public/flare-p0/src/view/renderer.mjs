...
export class Renderer {
 ...
 tile(q,hero){
  ...
  // Fade a foreground wall covering the warrior's body, without changing its collision.
 ...
 animation(actor,sim){
  const spec=this.map.sprites[actor.sprite],a=spec.animations[actor.state]||spec.animations.stance;
  const f=frameIndex(a,(sim.tick-actor.stateTick)/60);return a.entries[`${f}:${actor.dir}`];
 }
 actor(actor,sim){
  const g=this.ctx,v=this.camera,p=this.screen(actor.x,actor.y),f=this.animation(actor,sim);if(!f)return;
  const k=v.s*(actor.sprite==='goblin'?.90:1);const dead=actor.hp<=0;
  ...
  g.globalAlpha=dead?.62:1;g.drawImage(this.atlases[actor.sprite],f.x,f.y,f.w,f.h,p.x-f.ox*k,p.y-f.oy*k,f.dw*k,f.dh*k);g.globalAlpha=1;
 }
 diamond(x,y,fill,stroke){
  const g=this.ctx,p=this.screen(x+.5,y+.5),w=this.map.tileWidth*this.camera.s/2,h=this.map.tileHeight*this.camera.s/2;
  g.beginPath();g.moveTo(p.x,p.y-h);g.lineTo(p.x+w,p.y);g.lineTo(p.x,p.y+h);g.lineTo(p.x-w,p.y);g.closePath();if(fill){g.fillStyle=fill;g.fill();}if(stroke){g.strokeStyle=stroke;g.lineWidth=.7;g.stroke();}
 }
 marker(p,label,color,size=9){
  const g=this.ctx,q=this.screen(p.x,p.y);g.fillStyle='rgba(5,10,13,.85)';g.strokeStyle=color;g.lineWidth=1.5;
  g.beginPath();g.ellipse(q.x,q.y,size,size*.52,0,0,Math.PI*2);g.fill();g.stroke();
  g.font='600 11px system-ui';g.textAlign='center';g.fillStyle=color;g.fillText(label,q.x,q.y-12);
 }
 draw(sim,elapsed,dt,reducedMotion=false){
  ...
  if(this.showCollision)for(let y=0;y<this.map.height;y++)for(let x=0;x<this.map.width;x++){
   const c=this.map.collision[y*this.map.width+x];this.diamond(x,y,[1,2,3,4].includes(c)?'rgba(242,84,94,.28)':'rgba(111,213,190,.10)','rgba(255,255,255,.18)');
  }
  ...
  const exitOpen=sim.enemies.every(e=>e.hp<=0)&&sim.loot.every(l=>l.collected);
  this.marker(sim.exit,exitOpen?'EXIT OPEN':'EXIT',exitOpen?'#88dab8':'#8a989c',18);
  for(const i of sim.items)if(!i.collected)this.marker(i,'+10 HP','#df8999',9);
  for(const l of sim.loot)if(!l.collected)this.marker(l,`+${l.amount}`,'#ecd599',8);
  ...
  for(const q of list)if(q.kind==='tile')this.tile(q,hero);else this.actor(q.a,sim);
  for(const q of this.foreground)this.tile(q,hero);
  for(const a of [sim.hero,...sim.enemies])if(a.hp>0){
   const p=this.screen(a.x,a.y),f=this.animation(a,sim),height=(f?.oy||150)*this.camera.s,w=a.id==='hero'?42:34;
   const y=p.y-height-9;g.fillStyle='rgba(3,7,8,.85)';g.fillRect(p.x-w/2-1,y-1,w+2,6);g.fillStyle=a.id==='hero'?'#92d5c3':'#cf7772';g.fillRect(p.x-w/2,y,w*a.hp/a.maxHp,4);
  }
  for(let i=sim.events.length-1;i>=0;i--){const e=sim.events[i],age=(sim.tick-e.tick)/60;if(age>.7)break;if(!['damage','pickup'].includes(e.type))continue;
   const p=this.screen(e.x,e.y);g.globalAlpha=1-age/.7;g.font='700 14px system-ui';g.textAlign='center';g.fillStyle=e.type==='damage'?'#ffcfb5':e.kind==='gold'?'#f5d998':'#a4e0c7';
   g.strokeStyle='#101419';g.lineWidth=3;const text=(e.type==='damage'?'-':'+')+e.value;g.strokeText(text,p.x+18,p.y-30-age*26);g.fillText(text,p.x+18,p.y-30-age*26);g.globalAlpha=1;
  }
 }
}
