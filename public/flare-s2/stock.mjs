...
export async function loadStockRoom(id){if(!ROOMS[id])throw Error('Unknown room');if(!roomCache.has(id))roomCache.set(id,(async()=>{const spec=ROOMS[id],map=parseMap(await text(RAW+spec.source));map.id=id;map.name=spec.name;const packed=await tileAtlas(map);map.tiles=packed.frames;return{map,tiles:packed.canvas,spec};})());return roomCache.get(id);}
function normalizeAnimations(parsed){
  ...
  const a=parsed.animations,stance=a.stance;
  if(stance){a.run=a.run||a.run_alt||stance;a.attack=a.attack||a.swing||a.melee||stance;a.hit=a.hit||stance;a.die=a.die||a.death||stance;}
  return parsed;
}
async function stockSprite(configPath){const parsed=normalizeAnimations(parseAnimation(await text(`${RAW}mods/fantasycore/${configPath}`)));const atlas=await image(`${RAW}mods/fantasycore/${parsed.image}`);return{spec:{animations:parsed.animations},atlas};}
export async function loadActorPack(){if(!spriteCache.promise)spriteCache.promise=(async()=>{const [hero,goblin,skeleton]=await Promise.all([stockSprite('animations/npcs/knight.txt'),stockSprite('animations/enemies/goblin.txt'),stockSprite('animations/enemies/skeleton.txt')]);return{sprites:{warrior:hero.spec,goblin:goblin.spec,skeleton:skeleton.spec},atlases:{warrior:hero.atlas,goblin:goblin.atlas,skeleton:skeleton.atlas}};})();return spriteCache.promise;}
