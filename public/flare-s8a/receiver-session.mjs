...
export function advanceReceiver(session,event,payload={}){
  ...
  switch(event){
    ...
    case 'USE_DUNGEON':if(!session.roomId)throw new Error('Choose a dungeon first');return patch(session,{journey:transitionJourney(session.journey,'USE_DUNGEON')});
    case 'CUSTOMIZE':return patch(session,{journey:transitionJourney(session.journey,'CUSTOMIZE')});
    ...
    case 'RUN_AGAIN':return patch(session,{journey:transitionJourney(session.journey,'RUN_AGAIN'),lastResult:null,score:null,reward:null});
    case 'EDIT_DUNGEON':return patch(session,{journey:transitionJourney(session.journey,'EDIT_DUNGEON')});
